...
static inline const char* error_get_message(err_t err) {
...
#define ASSERT_GOTO(cond, label)                        do { if (!(cond)) goto label; } while(0)
...
#define CHECK_ERROR(expr)                       do { err_t err = (expr); if (err != ERROR_SUCCESS) { LOG_FATAL_ERROR("expression " ANSI_COLOR_WHITE_BOLD "`%s`" ANSI_COLOR_WHITE " returned an error " ANSI_COLOR_GRAY "(%s) [%d] at %s:%d:%s()", #expr, error_get_message(err), err, __FILENAME__, __LINE__, __FUNCTION__); exit(-1); } } while(0)
#define CHECK_ERROR_RETURN(expr)                do { err_t err = (expr); if (err != ERROR_SUCCESS) return; } while(0)
#define CHECK_ERROR_RETURN_VALUE(expr)          do { err_t err = (expr); if (err != ERROR_SUCCESS) return (value); } while(0)
#define CHECK_ERROR_PROPAGATE(expr)             do { err_t err = (expr); if (err != ERROR_SUCCESS) return err; } while(0)
