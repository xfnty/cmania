...
Model LoadModel(const char *fileName)
{
...
#if defined(SUPPORT_MESH_GENERATION)
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MESH: [%s] Failed to load mesh data, default to cube mesh", fileName);
        model.meshes[0] = GenMeshCube(1.0f, 1.0f, 1.0f);
#else
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MESH: [%s] Failed to load mesh data", fileName);
#endif
    }
    else
    {
    ...
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MATERIAL: [%s] Failed to load material data, default to white material", fileName);
...
void UnloadModel(Model model)
{
    ...
    TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: Unloaded model (and meshes) from RAM and VRAM");
}
...
void UploadMesh(Mesh *mesh, bool dynamic)
{
    if (mesh->vaoId > 0)
    {
        ...
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "VAO: [ID %i] Trying to re-load an already loaded mesh", mesh->vaoId);
        return;
    }
    ...
    if (mesh->vaoId > 0) TRACELOG(RAYLIB_LOG_LEVEL_INFO, "VAO: [ID %i] Mesh uploaded successfully to VRAM (GPU)", mesh->vaoId);
    else TRACELOG(RAYLIB_LOG_LEVEL_INFO, "VBO: Mesh uploaded successfully to VRAM (GPU)");
...
Material *LoadMaterials(const char *fileName, int *materialCount)
{
    ...
    if (IsFileExtension(fileName, ".mtl"))
    {
        ...
        if (result != TINYOBJ_SUCCESS) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MATERIAL: [%s] Failed to parse materials file", fileName);
...
#else
    TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to load material file", fileName);
#endif
...
{
    if (meshId >= model->meshCount) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MESH: Id greater than mesh count");
    else if (materialId >= model->materialCount) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MATERIAL: Id greater than material count");
...
void UpdateModelAnimation(Model model, ModelAnimation anim, int frame)
{
    if ((anim.frameCount > 0) && (anim.bones != NULL) && (anim.framePoses != NULL))
    {
        ...
        for (int m = 0; m < model.meshCount; m++)
        {
            ...
            {
                TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: UpdateModelAnimation(): Mesh %i has no connection to bones", m);
                continue;
            }
...
Mesh GenMeshSphere(float radius, int rings, int slices)
{
    ...
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MESH: Failed to generate mesh: sphere");
...
Mesh GenMeshHemiSphere(float radius, int rings, int slices)
{
    ...
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MESH: Failed to generate mesh: hemisphere");
...
Mesh GenMeshCylinder(float radius, float height, int slices)
{
    ...
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MESH: Failed to generate mesh: cylinder");
...
Mesh GenMeshCone(float radius, float height, int slices)
{
    ...
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MESH: Failed to generate mesh: cone");
...
Mesh GenMeshTorus(float radius, float size, int radSeg, int sides)
{
    ...
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MESH: Failed to generate mesh: torus");
...
Mesh GenMeshKnot(float radius, float size, int radSeg, int sides)
{
    ...
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MESH: Failed to generate mesh: knot");
...
void GenMeshTangents(Mesh *mesh)
{
    ...
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MESH: Tangents generation requires texcoord vertex attribute data");
        return;
    }
    ...
    TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MESH: Tangents data computed and uploaded for provided mesh");
}
...
static void BuildPoseFromParentJoints(BoneInfo *bones, int boneCount, Transform *transforms)
{
    for (int i = 0; i < boneCount; i++)
    {
        if (bones[i].parent >= 0)
        {
            if (bones[i].parent > i)
            {
                TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "Assumes bones are toplogically sorted, but bone %d has parent %d. Skipping.", i, bones[i].parent);
                continue;
            }
...
static Model LoadOBJ(const char *fileName)
{
    ...
    if (fileText != NULL)
    {
        ...
        {
            TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Failed to change working directory", workingDir);
        }

        unsigned int flags = TINYOBJ_FLAG_TRIANGULATE;
        int ret = tinyobj_parse_obj(&attrib, &meshes, &meshCount, &materials, &materialCount, fileText, dataSize, flags);

        if (ret != TINYOBJ_SUCCESS) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Failed to load OBJ data", fileName);
        else TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: [%s] OBJ data loaded successfully: %i meshes/%i materials", fileName, meshCount, materialCount);
        ...
        if (materialCount > 0)
        {
            ...
            model.materials = (Material *)RL_CALLOC(model.materialCount, sizeof(Material));
            TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: model has %i material meshes", materialCount);
        }
        else
        {
            model.meshCount = 1;
            TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: No materials, putting all meshes in a default material");
        }
        ...
        {
            TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Failed to change working directory", currentDir);
        }
    }
...
static Model LoadIQM(const char *fileName)
{
    ...
    */
    ...
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] IQM file is not a valid model", fileName);
        return model;
    }

    if (iqmHeader->version != IQM_VERSION)
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] IQM file version not supported (%i)", fileName, iqmHeader->version);
        return model;
    }
    ...
    for (int i = 0; i < model.meshCount; i++)
    {
        ...
        TRACELOG(RAYLIB_LOG_LEVEL_DEBUG, "MODEL: [%s] mesh name (%s), material (%s)", fileName, name, material);
...
static Image LoadImageFromCgltfImage(cgltf_image *cgltfImage, const char *texPath)
{
    ...
    if (cgltfImage->uri != NULL)     // Check if image data is provided as an uri (base64 or path)
    {
            ...
            (cgltfImage->uri[4] == ':'))     // Check if image is provided as base64 text data
        {
            ...
            if (cgltfImage->uri[i] == 0) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: glTF data URI is not a valid image");
            else
            {
    ...
    else if (cgltfImage->buffer_view->buffer->data != NULL)    // Check if image is provided as data buffer
    {
        ...
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: glTF image data MIME type not recognized", TextFormat("%s/%s", texPath, cgltfImage->uri));
...
static Model LoadGLTF(const char *fileName)
{
    ...
    ***********************************************************************************************/
    ...
    if (result == cgltf_result_success)
    {
        if (data->file_type == cgltf_file_type_glb) TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: [%s] Model basic data (glb) loaded successfully", fileName);
        else if (data->file_type == cgltf_file_type_gltf) TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: [%s] Model basic data (glTF) loaded successfully", fileName);
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Model format not recognized", fileName);

        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Meshes count: %i", data->meshes_count);
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Materials count: %i (+1 default)", data->materials_count);
        TRACELOG(RAYLIB_LOG_LEVEL_DEBUG, "    > Buffers count: %i", data->buffers_count);
        TRACELOG(RAYLIB_LOG_LEVEL_DEBUG, "    > Images count: %i", data->images_count);
        TRACELOG(RAYLIB_LOG_LEVEL_DEBUG, "    > Textures count: %i", data->textures_count);
        ...
        if (result != cgltf_result_success) TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: [%s] Failed to load mesh/material buffers", fileName);
            ...
            for (unsigned int p = 0; p < data->meshes[i].primitives_count; p++)
            {
                ...
                for (unsigned int j = 0; j < data->meshes[i].primitives[p].attributes_count; j++)
                {
                    ...
                    if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_position)      // POSITION
                    {
                        ...
                        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Vertices attribute data format not supported, use vec3 float", fileName);
                    }
                    else if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_normal)   // NORMAL
                    {
                        ...
                        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Normal attribute data format not supported, use vec3 float", fileName);
                    }
                    else if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_tangent)   // TANGENT
                    {
                        ...
                        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Tangent attribute data format not supported, use vec4 float", fileName);
                    }
                    else if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_texcoord) // TEXCOORD_0
                    {
                        ...
                        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Texcoords attribute data format not supported, use vec2 float", fileName);
                    }
                    else if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_color)    // COLOR_0
                    {
                        ...
                        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Color attribute data format not supported", fileName);
                    }
                ...
                if (data->meshes[i].primitives[p].indices != NULL)
                {
                    ...
                    else if (attribute->component_type == cgltf_component_type_r_32u)
                    {
                        ...
                        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Indices data converted from u32 to u16, possible loss of data", fileName);

                        RL_FREE(temp);
                    }
                    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Indices data format not supported, use u16", fileName);
                }
            ...
            for (unsigned int p = 0; p < data->meshes[i].primitives_count; p++)
            {
                ...
                for (unsigned int j = 0; j < data->meshes[i].primitives[p].attributes_count; j++)
                {
                    ...
                    if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_joints)        // JOINTS_n (vec4: 4 bones max per vertex / u8, u16)
                    {
                        ...
                        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Joint attribute data format not supported, use vec4 u8", fileName);
                    }
                    else if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_weights)  // WEIGHTS_n (vec4 / u8, u16, f32)
                    {
                        ...
                        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Joint weight attribute data format not supported, use vec4 float", fileName);
                    }
                }
    ...
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Failed to load glTF data", fileName);
...
static ModelAnimation *LoadModelAnimationsGLTF(const char *fileName, unsigned int *animCount)
{
    ...
    if (result != cgltf_result_success)
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Failed to load glTF data", fileName);
        *animCount = 0;
        return NULL;
    }

    result = cgltf_load_buffers(&options, data, fileName);
    if (result != cgltf_result_success) TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: [%s] Failed to load animation buffers", fileName);
            ...
            for (unsigned int i = 0; i < data->animations_count; i++)
            {
                ...
                for (unsigned int j = 0; j < animData.channels_count; j++)
                {
                    ...
                    if (animData.channels[j].sampler->interpolation == cgltf_interpolation_type_linear)
                    {
                        ...
                        {
                            TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Unsupported target_path on channel %d's sampler for animation %d. Skipping.", fileName, j, i);
                        }
                    }
                    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Only linear interpolation curves are supported for GLTF animation.", fileName);
                    ...
                    if (!r)
                    {
                        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Failed to load input time", fileName);
                        continue;
                    }
                ...
                for (int j = 0; j < animations[i].frameCount; j++)
                {
                    ...
                    for (int k = 0; k < animations[i].boneCount; k++)
                    {
                        ...
                        if (boneChannels[k].translate)
                        {
                            ...
                            {
                                TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: [%s] Failed to load translate pose data for bone %s", fileName, animations[i].bones[k].name);
                            }
                        }
                        ...
                        if (boneChannels[k].rotate)
                        {
                            ...
                            {
                                TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: [%s] Failed to load rotate pose data for bone %s", fileName, animations[i].bones[k].name);
                            }
                        }
                        ...
                        if (boneChannels[k].scale)
                        {
                            ...
                            {
                                TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: [%s] Failed to load scale pose data for bone %s", fileName, animations[i].bones[k].name);
                            }
                        }
                ...
                TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: [%s] Loaded animation: %s (%d frames, %fs)", fileName, animData.name, animations[i].frameCount, animDuration);
...
static Model LoadVOX(const char *fileName)
{
    ...
    if (fileData == 0)
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Failed to load VOX file", fileName);
        return model;
    }
    ...
    if (ret != VOX_SUCCESS)
    {
        ...
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Failed to load VOX data", fileName);
        return model;
    }
    else
    {
        ...
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: [%s] VOX data loaded successfully : %i vertices/%i meshes", fileName, nbvertices, meshescount);
    }
...
static Model LoadM3D(const char *fileName)
{
    ...
    if (fileData != NULL)
    {
        ...
        {
            TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "MODEL: [%s] Failed to load M3D data, error code %d", fileName, m3d ? m3d->errcode : -2);
            if (m3d) m3d_free(m3d);
            UnloadFileData(fileData);
            return model;
        }
        else TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: [%s] M3D data loaded successfully: %i faces/%i materials", fileName, m3d->numface, m3d->nummaterial);
        ...
        if (m3d->nummaterial > 0)
        {
            ...
            TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: model has %i material meshes", model.materialCount);
        }
        else
        {
...
static ModelAnimation *LoadModelAnimationsM3D(const char *fileName, unsigned int *animCount)
{
    ...
    *animCount = 0;
    ...
    if (fileData != NULL)
    {
        ...
        else TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: [%s] M3D data loaded successfully: %i animations, %i bones, %i skins", fileName,
        ...
        *animCount = m3d->numaction;
        ...
        for (unsigned int a = 0; a < m3d->numaction; a++)
        {
            ...
            TRACELOG(RAYLIB_LOG_LEVEL_INFO, "MODEL: [%s] animation #%i: %i msec, %i frames", fileName, a, m3d->action[a].durationmsec, animations[a].frameCount);
