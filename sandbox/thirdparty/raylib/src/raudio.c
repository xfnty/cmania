...
typedef enum {
    RAYLIB_LOG_LEVEL_ALL = 0,        // Display all logs
    RAYLIB_LOG_LEVEL_TRACE,          // Trace logging, intended for internal use only
    RAYLIB_LOG_LEVEL_DEBUG,          // Debug logging, used for internal debugging, it should be disabled on release builds
    RAYLIB_LOG_LEVEL_INFO,           // Info logging, used for program execution info
    RAYLIB_LOG_LEVEL_WARNING,        // Warning logging, used on recoverable failures
...
void InitAudioDevice(void)
{
    ...
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "AUDIO: Failed to initialize context");
        return;
    }
    ...
    if (result != MA_SUCCESS)
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "AUDIO: Failed to initialize playback device");
    ...
    if (result != MA_SUCCESS)
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "AUDIO: Failed to start playback device");
    ...
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "AUDIO: Failed to create mutex for mixing");
        ma_device_uninit(&AUDIO.System.device);
        ma_context_uninit(&AUDIO.System.context);
        return;
    }

    TRACELOG(RAYLIB_LOG_LEVEL_INFO, "AUDIO: Device initialized successfully");
    TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Backend:       miniaudio / %s", ma_get_backend_name(AUDIO.System.context.backend));
    TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Format:        %s -> %s", ma_get_format_name(AUDIO.System.device.playback.format), ma_get_format_name(AUDIO.System.device.playback.internalFormat));
    TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Channels:      %d -> %d", AUDIO.System.device.playback.channels, AUDIO.System.device.playback.internalChannels);
    TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Sample rate:   %d -> %d", AUDIO.System.device.sampleRate, AUDIO.System.device.playback.internalSampleRate);
    TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Periods size:  %d", AUDIO.System.device.playback.internalPeriodSizeInFrames*AUDIO.System.device.playback.internalPeriods);
...
void CloseAudioDevice(void)
{
    if (AUDIO.System.isReady)
    {
        ...
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "AUDIO: Device closed successfully");
    }
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "AUDIO: Device could not be closed, not currently initialized");
}
...
AudioBuffer *LoadAudioBuffer(ma_format format, ma_uint32 channels, ma_uint32 sampleRate, ma_uint32 sizeInFrames, int usage)
{
    ...
    if (audioBuffer == NULL)
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "AUDIO: Failed to allocate memory for buffer");
        return NULL;
    }
    ...
    if (result != MA_SUCCESS)
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "AUDIO: Failed to create data conversion pipeline");
        RL_FREE(audioBuffer);
        return NULL;
    }
...
Wave LoadWaveFromMemory(const char *fileType, const unsigned char *fileData, int dataSize)
{
    ...
    else if (strcmp(fileType, ".wav") == 0)
    {
        ...
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "WAVE: Failed to load WAV data");
    ...
    else if (strcmp(fileType, ".ogg") == 0)
    {
        ...
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "WAVE: Failed to load OGG data");
    }
#endif
#if defined(SUPPORT_FILEFORMAT_MP3)
    else if (strcmp(fileType, ".mp3") == 0)
    {
        ...
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "WAVE: Failed to load MP3 data");
    ...
    else if (strcmp(fileType, ".qoa") == 0)
    {
        ...
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "WAVE: Failed to load QOA data");
    ...
    else if (strcmp(fileType, ".flac") == 0)
    {
        ...
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "WAVE: Failed to load FLAC data");
    }
#endif
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "WAVE: Data format not supported");

    TRACELOG(RAYLIB_LOG_LEVEL_INFO, "WAVE: Data loaded successfully (%i Hz, %i bit, %i channels)", wave.sampleRate, wave.sampleSize, wave.channels);
...
Sound LoadSoundFromWave(Wave wave)
{
    ...
    if (wave.data != NULL)
    {
        ...
        if (frameCount == 0) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "SOUND: Failed to get frame count for format conversion");
        ...
        if (audioBuffer == NULL)
        {
            TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "SOUND: Failed to create buffer");
        ...
        if (frameCount == 0) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "SOUND: Failed format conversion");
...
{
    RL_FREE(wave.data);
    //TRACELOG(RAYLIB_LOG_LEVEL_INFO, "WAVE: Unloaded wave data from RAM");
}
...
void UnloadSound(Sound sound)
{
    ...
    //TRACELOG(RAYLIB_LOG_LEVEL_INFO, "SOUND: Unloaded sound data from RAM");
}
...
bool ExportWave(Wave wave, const char *fileName)
{
    ...
    else if (IsFileExtension(fileName, ".qoa"))
    {
        ...
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "AUDIO: Wave data must be 16 bit per sample for QOA format export");
    }
#endif
    ...
    if (success) TRACELOG(RAYLIB_LOG_LEVEL_INFO, "FILEIO: [%s] Wave data exported successfully", fileName);
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to export wave data", fileName);
...
bool ExportWaveAsCode(Wave wave, const char *fileName)
{
    ...
    if (success != 0) TRACELOG(RAYLIB_LOG_LEVEL_INFO, "FILEIO: [%s] Wave as code exported successfully", fileName);
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to export wave as code", fileName);
...
void WaveFormat(Wave *wave, int sampleRate, int sampleSize, int channels)
{
    ...
    if (frameCount == 0)
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "WAVE: Failed to get frame count for format conversion");
        return;
    }
    ...
    if (frameCount == 0)
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "WAVE: Failed format conversion");
        return;
    }
...
void WaveCrop(Wave *wave, int initSample, int finalSample)
{
    ...
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "WAVE: Crop range out of bounds");
}
...
Music LoadMusicStream(const char *fileName)
{
...
#endif
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "STREAM: [%s] File format not supported", fileName);
    ...
    if (!musicLoaded)
    {
        ...
        music.ctxData = NULL;
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: [%s] Music file could not be opened", fileName);
    }
    else
    {
        // Show some music stream info
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "FILEIO: [%s] Music file loaded successfully", fileName);
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Sample rate:   %i Hz", music.stream.sampleRate);
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Sample size:   %i bits", music.stream.sampleSize);
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Channels:      %i (%s)", music.stream.channels, (music.stream.channels == 1)? "Mono" : (music.stream.channels == 2)? "Stereo" : "Multi");
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Total frames:  %i", music.frameCount);
    }
...
Music LoadMusicStreamFromMemory(const char *fileType, const unsigned char *data, int dataSize)
{
...
#endif
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "STREAM: Data format not supported");
    ...
    if (!musicLoaded)
    {
        ...
        music.ctxData = NULL;
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: Music data could not be loaded");
    }
    else
    {
        // Show some music stream info
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "FILEIO: Music data loaded successfully");
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Sample rate:   %i Hz", music.stream.sampleRate);
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Sample size:   %i bits", music.stream.sampleSize);
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Channels:      %i (%s)", music.stream.channels, (music.stream.channels == 1)? "Mono" : (music.stream.channels == 2)? "Stereo" : "Multi");
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "    > Total frames:  %i", music.frameCount);
...
AudioStream LoadAudioStream(unsigned int sampleRate, unsigned int sampleSize, unsigned int channels)
{
    ...
    if (stream.buffer != NULL)
    {
        ...
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "STREAM: Initialized successfully (%i Hz, %i bit, %s)", stream.sampleRate, stream.sampleSize, (stream.channels == 1)? "Mono" : "Stereo");
    }
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "STREAM: Failed to load audio buffer, stream could not be created");
...
void UnloadAudioStream(AudioStream stream)
{
    ...
    TRACELOG(RAYLIB_LOG_LEVEL_INFO, "STREAM: Unloaded audio stream data from RAM");
}
...
void UpdateAudioStream(AudioStream stream, const void *data, int frameCount)
{
    if (stream.buffer != NULL)
    {
        if (stream.buffer->isSubBufferProcessed[0] || stream.buffer->isSubBufferProcessed[1])
        {
            ...
            else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "STREAM: Attempting to write too many frames to buffer");
        }
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "STREAM: Buffer not available for updating");
    }
}
...
static void OnLog(void *pUserData, ma_uint32 level, const char *pMessage)
{
    TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "miniaudio: %s", pMessage);   // All log messages from miniaudio are errors
}
        ...
        if (file != NULL)
        {
            ...
            if (size > 0)
            {
                ...
                *bytesRead = count;
                ...
                if (count != size) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: [%s] File partially loaded", fileName);
                else TRACELOG(RAYLIB_LOG_LEVEL_INFO, "FILEIO: [%s] File loaded successfully", fileName);
            }
            else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to read file", fileName);

            fclose(file);
        }
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to open file", fileName);
    }
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: File name provided is not valid");
...
static bool SaveFileData(const char *fileName, void *data, unsigned int bytesToWrite)
{
    if (fileName != NULL)
    {
        ...
        if (file != NULL)
        {
            ...
            if (count == 0) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to write file", fileName);
            else if (count != bytesToWrite) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: [%s] File partially written", fileName);
            else TRACELOG(RAYLIB_LOG_LEVEL_INFO, "FILEIO: [%s] File saved successfully", fileName);
...
static bool SaveFileText(const char *fileName, char *text)
{
    if (fileName != NULL)
    {
        ...
        if (file != NULL)
        {
            ...
            if (count == 0) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to write text file", fileName);
            else TRACELOG(RAYLIB_LOG_LEVEL_INFO, "FILEIO: [%s] Text file saved successfully", fileName);

            fclose(file);
        }
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to open text file", fileName);
    }
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: File name provided is not valid");
}
#endif
