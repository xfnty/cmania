...
Image LoadImageFromMemory(const char *fileType, const unsigned char *fileData, int dataSize)
{
    ...
    else if (strcmp(fileType, ".hdr") == 0)
    {
        ...
        if (fileData != NULL)
        {
            ...
            {
                TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: HDR file format not supported");
                UnloadImage(image);
            }
        }
#endif
...
#endif
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Data format not supported");

    if (image.data != NULL) TRACELOG(RAYLIB_LOG_LEVEL_INFO, "IMAGE: Data loaded successfully (%ix%i | %s | %i mipmaps)", image.width, image.height, rlGetPixelFormatName(image.format), image.mipmaps);
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Failed to load image data");
...
Image LoadImageFromTexture(Texture2D texture)
{
    ...
    if (texture.format < PIXELFORMAT_COMPRESSED_DXT1_RGB)
    {
        ...
        if (image.data != NULL)
        {
            ...
            image.format = PIXELFORMAT_UNCOMPRESSED_R8G8B8A8;
#endif
            TRACELOG(RAYLIB_LOG_LEVEL_INFO, "TEXTURE: [ID %i] Pixel data retrieved successfully", texture.id);
        }
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "TEXTURE: [ID %i] Failed to retrieve pixel data", texture.id);
    }
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "TEXTURE: [ID %i] Failed to retrieve compressed pixel data", texture.id);
...
bool ExportImage(Image image, const char *fileName)
{
    ...
    else if (IsFileExtension(fileName, ".qoi"))
    {
        ...
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Image pixel format must be R8G8B8 or R8G8B8A8");
    ...
    if (success != 0) TRACELOG(RAYLIB_LOG_LEVEL_INFO, "FILEIO: [%s] Image exported successfully", fileName);
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to export image", fileName);
...
bool ExportImageAsCode(Image image, const char *fileName)
{
    ...
    if (success != 0) TRACELOG(RAYLIB_LOG_LEVEL_INFO, "FILEIO: [%s] Image as code exported successfully", fileName);
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FILEIO: [%s] Failed to export image as code", fileName);
...
void ImageCrop(Image *image, Rectangle crop)
{
    ...
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Failed to crop, rectangle out of bounds");
        return;
    }

    if (image->mipmaps > 1) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "Image manipulation only applied to base mipmap level");
    if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "Image manipulation not supported for compressed formats");
    else
    {
        int bytesPerPixel = GetPixelDataSize(1, 1, image->format);
...
void ImageFormat(Image *image, int newFormat)
{
    ...
    if ((newFormat != 0) && (image->format != newFormat))
    {
        ...
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Data format is compressed, can not be converted");
    }
}
...
Image ImageText(const char *text, int fontSize, Color color)
{
    ...
    TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: ImageTextEx() requires module: rtext");
#endif
    return imText;
}
...
Image ImageTextEx(Font font, const char *text, float fontSize, float spacing, Color tint)
{
    ...
    if (textSize.y != imSize.y)
    {
        ...
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "IMAGE: Text scaled by factor: %f", scaleFactor);
...
void ImageAlphaMask(Image *image, Image alphaMask)
{
    ...
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Alpha mask must be same size as image");
    }
    else if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Alpha mask can not be applied to compressed data formats");
    }
    else
    {
...
void ImageMipmaps(Image *image)
{
    ...
    if (image->mipmaps < mipCount)
    {
        ...
        else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Mipmaps required memory could not be allocated");
    ...
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Mipmaps already available");
}
...
void ImageDither(Image *image, int rBpp, int gBpp, int bBpp, int aBpp)
{
    ...
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Compressed data formats can not be dithered");
        return;
    }

    if ((rBpp + gBpp + bBpp + aBpp) > 16)
    {
        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Unsupported dithering bpps (%ibpp), only 16bpp or lower modes supported", (rBpp+gBpp+bBpp+aBpp));
    }
    else
    {
        Color *pixels = LoadImageColors(*image);
        ...
        {
            TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Format is already 16bpp or lower, dithering could have no effect");
        }
        ...
        {
            image->format = 0;
            TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Unsupported dithered OpenGL internal format: %ibpp (R%iG%iB%iA%i)", (rBpp+gBpp+bBpp+aBpp), rBpp, gBpp, bBpp, aBpp);
        }
...
Color *LoadImageColors(Image image)
{
    ...
    if (image.format >= PIXELFORMAT_COMPRESSED_DXT1_RGB) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Pixel data retrieval not supported for compressed image formats");
    else
    {
        if ((image.format == PIXELFORMAT_UNCOMPRESSED_R32) ||
            (image.format == PIXELFORMAT_UNCOMPRESSED_R32G32B32) ||
            (image.format == PIXELFORMAT_UNCOMPRESSED_R32G32B32A32)) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Pixel format converted from 32bit to 8bit per channel");
            ...
            if (pixels[i].a > 0)
            {
                ...
                if (!colorInPalette)
                {
                    ...
                    if (palCount >= maxPaletteSize)
                    {
                        ...
                        TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Palette is greater than %i colors", maxPaletteSize);
                    }
                }
            }
        }
...
Color GetImageColor(Image image, int x, int y)
{
    ...
    if ((x >=0) && (x < image.width) && (y >= 0) && (y < image.height))
    {
        switch (image.format)
        {
            ...
            default: TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "Compressed image format does not support color reading"); break;
        }
    }
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "Requested image pixel (%i, %i) out of bounds", x, y);
...
void ImageDraw(Image *dst, Image src, Rectangle srcRec, Rectangle dstRec, Color tint)
{
    ...
    if (dst->mipmaps > 1) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "Image drawing only applied to base mipmap level");
    if (dst->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "Image drawing not supported for compressed formats");
    else
    {
...
#else
    TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: ImageDrawText() requires module: rtext");
#endif
}
...
Texture2D LoadTextureFromImage(Image image)
{
    ...
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Data is not valid to load texture");
...
TextureCubemap LoadTextureCubemap(Image image, int layout)
{
    ...
    if (layout != CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        if (cubemap.id == 0) TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Failed to load cubemap image");

        UnloadImage(faces);
    }
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "IMAGE: Failed to detect cubemap image layout");
...
RenderTexture2D LoadRenderTexture(int width, int height)
{
    ...
    if (target.id > 0)
    {
        ...
        if (rlFramebufferComplete(target.id)) TRACELOG(RAYLIB_LOG_LEVEL_INFO, "FBO: [ID %i] Framebuffer object created successfully", target.id);

        rlDisableFramebuffer();
    }
    else TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "FBO: Framebuffer object can not be created");
...
void UnloadTexture(Texture2D texture)
{
    if (texture.id > 0)
    {
        ...
        TRACELOG(RAYLIB_LOG_LEVEL_INFO, "TEXTURE: [ID %i] Unloaded texture data from VRAM (GPU)", texture.id);
    }
}
...
void SetTextureFilter(Texture2D texture, int filter)
{
    switch (filter)
    {
        ...
        case TEXTURE_FILTER_TRILINEAR:
        {
            ...
            {
                TRACELOG(RAYLIB_LOG_LEVEL_WARNING, "TEXTURE: [ID %i] No mipmaps available for TRILINEAR texture filtering", texture.id);
